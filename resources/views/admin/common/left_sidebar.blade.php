<aside class="main-sidebar">
	...
	<!-- 	@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'properties'))
			<li class="{{ (Route::current()->uri() == 'admin/add-propertie') || (Route::current()->uri() == 'admin/add-properties') || (Route::current()->uri() == 'admin/listing/{id}/{step}') ? 'active' : ''  }}"><a href="{{ url('admin/add-properties') }}"><i class="fa fa-home"></i><span>Add Properties</span></a></li>
		@endif -->
	<!-- 	@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'properties'))
			<li class="{{ (Route::current()->uri() == 'admin/properties')  || (Route::current()->uri() == 'admin/listing/{id}/{step}') ? 'active' : ''  }}"><a href="{{ url('admin/properties') }}"><i class="fa fa-home"></i><span>Properties</span></a></li>
		@endif -->
	@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'properties'))
			<li class="{{ (Route::current()->uri() == 'admin/properties/users')  || (Route::current()->uri() == 'admin/listing/{id}/{step}') ? 'active' : ''  }}"><a href="{{ url('admin/properties/users') }}"><i class="fa fa-home"></i><span>Properties</span></a></li>
		@endif
		...
		@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_admin'))
			<li class="{{ (Route::current()->uri() == 'admin/admin-users') || (Route::current()->uri() == 'admin/add-admin') || (Route::current()->uri() == 'admin/edit-admin/{id}') ? 'active' : ''  }}">
				...
				<i class="fa fa-user-plus"></i> <span>Admin Users</span>
				</a>
			</li>
		@endif
		...
		<!-- Email Template Ends -->  
		@if(Helpers::has_permission(Auth::guard('admin')->user()->id, 'manage_messages'))
			<li class="{{ (Route::current()->uri() == 'admin/chat') ||  (Route::current()->uri() == 'admin/chat') ? 'active' : ''  }}">
				<a href="{{ url('admin/chat') }}">
				<i class="fa fa-user-plus"></i> <span>Chat Filter</span>
				</a>
			</li>
		@endif
		...
		@php
		$data = file_get_contents('http://bnbtoronto.ca/api/getreport');
		$decodedData = json_decode($data);

		@endphp
			<li class="{{ (Route::current()->uri() == 'bookings/report') ? 'active' : ''  }}"><a href="{{ url('admin/bookings/report') }}"><i class="fa fa-exclamation-triangle"></i><span>Customer Report ({{$decodedData}})</span></a></li>
		@endif
    </ul>
    </section>
</aside>