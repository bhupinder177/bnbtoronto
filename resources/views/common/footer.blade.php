...
<footer class="main-panel card border footer-bg p-4" id="footer">
    <div class="container-fluid container-fluid-90">
        <div class="row">
            <div class="col-lg-4 col-sm-4" >
                <h2 class="font-weight-700" style="margin-left: 20px;">{{ trans('messages.static_pages.hosting') }}</h2>
                <div class="row">
                    <div class="col p-0" style="margin-left: 20px;">
                        <ul class="mt-1" >
                            @if(isset($footer_first))
                              <li class="pt-3 text-16">
                                    <a href="https://bnbtoronto.ca/faqs" target="_blank">FAQS</a>
                                </li>
                          
                            @foreach($footer_first as $ff)
                                @if( $ff->faq == 0)
                                <li class="pt-3 text-16">
                                    <a href="{{ url($ff->url) }}" target="_blank">{{ $ff->name }}</a>
                                </li>
                                @endif 
                            @endforeach
                            @endif 
                ...
                <div class="row">
                    <div class="col p-0">
                        <ul class="mt-1">
                            ...
                            <li class="pt-3 text-16">
                                <a href="{{URL::to('/')}}/search?location={{ $pc->name }}&checkin={{ date('d-m-Y') }}&checkout={{ date('d-m-Y') }}&guest=1"  target="_new">{{ $pc->name }}</a>
                            </li>
                            @endforeach
                            @endif
                        </ul>
            ...
            <div class="col-4 ">
                <!-- <div class="row mt-5">
                    <div class="col-md-12 text-center">
                        <a href="{{ url('/') }}"  target="_new"><img src="{{$logo ?? ''}}" class="img-130x32" alt="logo"></a>
                    </div>
                </div> -->

                <div class="col-md-12">
                    <div class="social ">
                        <h2 class="font-weight-700">SOCIAL</h2>
                        <ul class=" text-center">
                            @if(isset($join_us))
                            @for($i=0; $i<count($join_us); $i++)
                            <li class="list-item">
                            <a class="social-icon  text-color text-18" target="_blank" href="{{ $join_us[$i]->value }}" aria-label="{{$join_us[$i]->name}}"  target="_new"><i class="fab fa-{{ str_replace('_','-',$join_us[$i]->name) }}"></i> {{$join_us[$i]->name}}</a>
                            </li>
                            @endfor
                            @endif  
                        </ul>
	...
	<div class="border-top container-fluid">
		<div class="row  justify-content-between p-2">
			<p class="col-lg-6 col-sm-6 mb-0 mt-4 text-14 text-left" style="width: 400px">
			© {{ date('Y') }} {{$site_name ?? ''}}. {{ trans('messages.home.all_rights_reserved') }}</p>
                <span class="col-lg-6 col-sm-6 mb-0 mt-4 text-14 text-right">$ CAD</span>
		</div>
	</div>
</footer>
