...
<div class="container mb-4 margin-top-85 min-height">
	<div class="d-flex justify-content-center">
		<div class="p-5 mt-5 mb-5 border w-450 login-bk" >
			...
			<p style="font-size: 13px;margin-bottom: 20px">Please login to continue</p>
		...
		<!-- 	<a href="{{ isset($facebook_url) ? $facebook_url:URL::to('facebookLogin') }}">
			...
			<form id="login_form" method="post" action="{{url('authenticate')}}"  accept-charset='UTF-8'>  
				...
				<div class="form-group col-sm-12 p-0">
					...
					<input type="email" class="form-control text-14 input-field" value="{{ old('email') }}" name="email" placeholder = "{{trans('messages.login.email')}}">
					</div>
					...
					<i class="fa fa-lock icon orange"></i>

					<input type="password" class="form-control text-14 input-field" value="" name="password" placeholder = "{{trans('messages.login.password')}}">
					</div>
				</div>
	...
	</div><center><p class="signup">{{trans('messages.login.do_not_have_an_account')}}<a href="{{URL::to('/')}}/signup" >
				<span class="orange">{{trans('messages.login.register')}}</span>
				</a></p></center>
</div>
@stop
