    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->addColumn('status', function ($bookings) {
             ...
             ->addColumn('reasons', function ($bookings) {
                return $bookings->reasons;
            })
            ->addColumn('action', function ($bookings) {
                    $details =  '<a href="' . url('admin/bookings/detail/' . $bookings->id) . '" class="btn btn-xs btn-primary" title="Detail View"><i class="fa fa-share"></i></a>&nbsp;';

                    $onhold = '<a href="' . url('admin/bookings/statuschange/' . $bookings->id) . '" class="btn btn-xs btn-warning cancel-warning" style="margin-left:3px;"><i class="glyphicon glyphicon-exclamation-sign" ></i></a>';
                

return $details.$onhold;

            })
            ->rawColumns(['host_name','guest_name','status','total_amount','property_name', 'action'])
            ->make(true);
    }
    ...
    public function query()
    {
        ...
        $bookings = Bookings::join('properties', function ($join) {
                        ...
                        ->select(['bookings.id as id', 'u.first_name as host_name', 'users.first_name as guest_name', 'bookings.property_id as property_id', 'properties.name as property_name', DB::raw('CONCAT(currency.symbol, bookings.total) AS total_amount'), 'bookings.status','bookings.reasons', 'bookings.created_at as created_at', 'bookings.updated_at as updated_at', 'bookings.start_date', 'bookings.end_date', 'bookings.guest', 'bookings.host_id', 'bookings.user_id', 'bookings.total', 'bookings.currency_code', 'bookings.service_charge', 'bookings.host_fee']);
        if (isset($user_id)) {
    ...
    public function html()
    {
        ...
        return $this->builder()
            ...
            ->addColumn(['data' => 'reasons', 'name' => 'bookings.reasons', 'title' => 'Reasons'])
