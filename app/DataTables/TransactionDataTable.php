...
class TransactionDataTable extends DataTable
{
    public function ajax()
    {
     ...
     ->addColumn('status', function ($transaction) {
            $guest = dateFormat($transaction->guest);
            return $guest;
        })
          ->addColumn('code', function ($transaction) {
            $code = dateFormat($transaction->code);
            return $code;
        })

             ->addColumn('propertyid', function ($transaction) {
            $propertyid = dateFormat($transaction->property_id);
            return $propertyid;
        })
    ...
    public function query()
    {
        ...
        })->select(['bookings.id as id', DB::raw('(bookings.total - bookings.service_charge) as amount'), 'bookings.currency_code AS c', 'currency.symbol AS symbol', 'payment_methods.name AS method', 'bookings.created_at AS date'])->where('bookings.host_id', '=', $user_id)->where('bookings.status', '=', 'Accepted');
    ...
    {
        return $this->builder()
        ->addColumn(['data' => 'guest', 'name' => 'guestname', 'title' => "Guest Name"])
        ->addColumn(['data' => 'code', 'name' => 'confirmcode', 'title' => "Confirmation Code"])
        ->addColumn(['data' => 'propertyid', 'name' => 'listing', 'title' => "Listing"])
        ...
        //->addColumn(['data' => 'amount', 'name' => 'amount', 'title' => trans('messages.withdraw.amount')])
         ->addColumn(['data' => 'amount', 'name' => 'amount', 'title' => "Total Payouts"])
