...
//end of counts code   
    ...
    {
            $reasons = $request->otherreason;
            if($request->otherreason1 !=""){
                $reasons = $request->otherreason1;
            }
        $cancel = Bookings::where('id', $request->id)->first();
        $cancel->status = "report";
        $cancel->reasons = $reasons;
        $cancel->reasonsid = 1;
        $cancel->save();
        
      $this->helper->one_time_message('success', "Report send is successfully sent.");
       return redirect('inbox');
    }


      public function getreport(Request $request){
        $cancel = Bookings::where('status', "Report")->get();
       return count( $cancel);
    }
}
