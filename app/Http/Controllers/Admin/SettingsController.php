...
public function socialLinksdelete(Request $request)
    {
         Settings::where('name', $request->id)->delete();
         $this->helper->one_time_message('success', 'Deleted Successfully');
         return redirect('admin/settings/social-links');

}
public function reasonlist(ReasonsDataTable $dataTable)
{
    return $dataTable->render('admin.reasons.view');
}
...
public function addreasons(Request $request)
    {
       ...
       //return $dataTable->render('admin.reasons.view');
        ...
        // DB::insert(DB::raw("INSERT INTO tbl_reasons(title,status) VALUES ('$title','$status')"));
        // $this->helper->one_time_message('success', 'Added Successfully');
        // return redirect('settings/add-reasons');
    }

    public function addsocialLinks(Request $request)
    {
        $field = str_replace(" ", "_", $request->socialname);
        $value = "";
        $type= "join_us";
        DB::insert(DB::raw("INSERT INTO settings(name,value,type) VALUES ('$field','$value','$type')"));
        $this->helper->one_time_message('success', 'Updated Successfully');
        return redirect('admin/settings/social-links');
    }
    ...
    public function socialLinks(Request $request)
    {
        if (! $request->isMethod('post')) {
              ...
              $data['result'] = $this->helper->key_value('name', 'value', $general);
              $result2 = Settings::where('type', 'join_us')->get();


foreach ($result2 as $value){
     $data['sociallist'][]= ['type' => 'text', 'class' => '', 'label' =>  str_replace("_", " ", ucfirst($value->name)), 'name' => $value->name, 'value' => $value->value];
    }
 ...
 $result3 = Settings::where('type', 'join_us')->get();

foreach ($result3 as $value2){
     // $data['sociallist'][]= ['type' => 'text', 'class' => '', 'label' => $value->name, 'name' => $value->name, 'value' => $value->value];
$test =$value2->name;

Settings::where(['name' => $value2->name])->update(['value' => $request->$test]);
   ...
   public function addReason(Request $request)
    {
        $data['s'] = 1;
          if(isset($request->id) || isset($request->saveid)){

            if(isset($request->saveid)){

            $amenityType =  ReportList::find($request->saveid);
            $amenityType->title = $request->title;
            $amenityType->status = $request->status;
            $amenityType->save();
            $this->helper->one_time_message('success', 'Update Successfully');
            return redirect('admin/settings/add-reasons');

            }else{
                 $amenityType = ReportList::where('id',$request->id)->first();
                 $data['id'] = $amenityType->id;
                 $data['title'] = $amenityType->title;
                 $data['status'] = $amenityType->status;
                 return view('admin.reasons.add', $data);
            }
    ...
    {
        ReportList::find($request->id)->delete();

        $this->helper->one_time_message('success', 'Deleted Successfully');
        return redirect('admin/settings/reasons');
    }
