...
class BookingsController extends Controller
{
    ...
    public function cancelbooking(Request $request){
        $whyonholds="";
        if(isset($request->whyonhold3)){
            $whyonholds = $request->whyonhold3;
        }
                $bookingsdetails = Bookings::find($request->id);
                $bookingsdetails->status  = $request->statustype;
                $bookingsdetails->reasons = $whyonholds;
                $bookingsdetails->save();

        $this->helper->one_time_message('success', 'Status Updated Successfully');
