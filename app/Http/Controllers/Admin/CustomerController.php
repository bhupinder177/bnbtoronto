...
class CustomerController extends Controller
{
 ...
 
    {
        ...
        $host = User::where('user_type', 'host' )->get();

        $data['host'] = count($host);

         $guest = User::where('user_type', 'guest' )->get();

        $data['guest'] = count($guest);

        $onhold = User::where('status', 'onhold' )->get();

        $data['onhold'] = count($onhold);
    ...
    public function chatfilter(ChatDataTable $dataTable)
 ...
 
    {
        ...
        if (isset(request()->customer)) {
            $data['customers'] = ChatFilter::where('users.id', request()->customer )->select('id', 'first_name', 'last_name')->get();
        } else {
            $data['customers'] = null;
        }
        ...
        return $dataTable->render('admin.chat.chat-filter', $data);
   ...
   public function addchat(Request $request, EmailController $email_controller)
    {
    ...
    public function customerprop(CustomerpropDataTable $dataTable)
 ...
 
    {
        ...
        if (isset(request()->customer)) {
            $data['customers'] = User::where('users.id', request()->customer )->select('id', 'first_name', 'last_name')->get();
             // $data['customers'] = User::where('user_type', 'host' )->get();
        } else {
            $data['customers'] = null;
        }
        
        $totalprop = count(Properties::get());
        $totalproponhold = count(Properties::where('status', 'Onhold' )->get());
        $totalpropunlisted = count(Properties::where('status', 'Unlisted' )->get());
       $totalpropnew = count(Properties::where('status', 'New' )->get());
        $host = User::where('user_type', 'host' )->get();
        $data['totalprop'] = $totalprop;
        $data['totalproponhold'] = $totalproponhold;
        $data['totalpropunlisted'] =$totalpropunlisted;
         $data['totalpropnew'] = $totalpropnew;
        $data['host'] = count($host);
        ...
        //     $data['allstatus']   = '';
        //     $data['allcustomers']   = '';
        //     return $dataTable->render('admin.properties.user_list', $data);
        // }
        // $pref = Settings::where('type', 'preferences')->get();
        // if (! empty($pref)) {
        //     foreach ($pref as $value) {
    ...
    public function update(Request $request)
    {
        ...
        if (! $request->isMethod('post')) { 
            ...
            if ($validator->fails()) {
                ...
                $user->last_name       = $request->last_name;
                $user->email           = $request->email;
                $user->reasons         = $request->reasontext;
                $user->status          = $request->status;
                $user->profile_image   = NULL;
                $formattedPhone        = str_replace('+' . $request->carrier_code, "", $request->formatted_phone);
                $user->phone           = !empty($request->phone) ? preg_replace("/[\s-]+/", "", $formattedPhone) : NULL;
