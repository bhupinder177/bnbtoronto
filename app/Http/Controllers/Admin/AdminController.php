   ...
   public function authenticate(Request $request)
    {
        ...
        if (!empty($admin->status)) {
          ...
          } elseif($admin->status == 'Onhold') {
            $this->helper->one_time_message('danger', 'You are Onhold.'); 
            return redirect('admin/login'); 
          } else {
    ...
    public function updateCustomer(Request $request) 
    {
      ...
      public function messagesCsv()
    { 
         
         return Excel::download(new MessageExport, 'message_sheet' . time() .'.xls');
    }
