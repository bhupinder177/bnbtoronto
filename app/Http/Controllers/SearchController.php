...
class SearchController extends Controller
{
    ...
    public function getimages(Request $request){
          $data   = PropertyPhotos::where('property_id', $request->id)->orderBy('serial', 'asc')
            ->get();
            $show = '<div class="sliderss">';
            foreach ($data as $photos) {
                 $show .="<div><center><img src='http://bnbtoronto.ca/public/images/property/".$request->id."/".$photos->photo."' style='width:250px;'></center></div>";
            }
             $show .= '</div>';
            return $show;
