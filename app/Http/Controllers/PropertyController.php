...
class PropertyController extends Controller
{
    ...
    {   
        ...
        if(isset($request->deletebtn)){
                ...
                $data['deletemsg'] = "Your Listing is already deleted";
   Properties::where('id',$request->deletebtn)->delete();


        }

        $data['status'] = $request->status;
        $data['properties'] = Properties::with('property_price', 'property_address','property_description')
        ...
        if ($step == 'basics') {
            if ($request->isMethod('post')) {
                if($request->bed_type==""){
                    ...
                    $bedtypesss =$request->bed_type;
                }
                $property                     = Properties::find($property_id);
                $property->bedrooms           = $request->bedrooms;
                $property->beds               = $request->beds;
                $property->bathrooms          = $request->bathrooms;
                $property->bed_type           = $bedtypesss;
            ...
            if ($request->isMethod('post')) {
                ...
                if ($validator->fails()) {
                    ...
                    $property_description->youtubelink     = $request->youtubelink;
                    $property_description->internal_name     = $request->internal_name;
                    $property_description->neighborhood_desc         = $request->neighborhood_desc;
                    $property_description->about_neighborhood   = $request->neighborhood_desc;
                    $property_description->wifi_details         = $request->wifidetails;
...
$data['description']       = PropertyDescription::where('property_id', $property_id)->first();
            ...
            if ($request->isMethod('post')) {
                ...
                $properties->status       = ( $properties->steps_completed == 0 ) ?  'Listed' : 'New';
                $properties->save();
