...
class LoginController extends Controller
{
    ...
    public function authenticate(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if (!empty($users)) {  
               ...
               if ($users->status == 'Onhold') {
                    $this->helper->one_time_message('danger', "User is onhold. Please contact the admin!");
                    return redirect('login');
                }elseif ($users->status != 'Inactive') {
                    if (Auth::attempt(['email' => $request->email, 'password' => $request->password], $remember)) {
                        
                        if($users->user_type == "host"){
                            $hostback = Settings::where('name', 'hostbackground')->first();
                            session(['hostbackground' => $hostback['value']]);
                        }
                        else{
                            $hostback = Settings::where('name', 'guestbackground')->first();
                            session(['hostbackground' => $hostback['value']]);
                        }
    ...
    public function emailtest(Request $request){
        $body_string="testing only";
           $data['content']    =   $content = $body_string;

            Mail::send('emails.custom_email', [], function ($m) use($request,$content){
                        $m
                          ->from("info@bnbtoronto.ca")
                          ->to('maluherby@gmail.com')
